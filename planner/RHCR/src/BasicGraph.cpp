...
int BasicGraph::get_direction_output(int from, int to) const
{
    ...
    {
        std::cout << "Error in get_direction_output: " << from << " to " << to << std::endl;
        return -1;
    }
}
...
void BasicGraph::save_heuristics_table(std::string fname)
{
    if (this->_save_heuristics_table)
    {
        ...
        // To preserve order, create a vector, store the heuristics there, and sort the elements by key
        std::vector<std::pair<int, std::vector<double>>> heuristics_vector(heuristics.begin(), heuristics.end());
        std::sort(heuristics_vector.begin(), heuristics_vector.end(),
            [](const std::pair<int, std::vector<double>>& a, const std::pair<int, std::vector<double>>& b) {
