...
void BeeGraph::preprocessing(bool consider_rotation, std::string log_dir)
{
	...
	std::ifstream myfile((log_dir).c_str());
	if (myfile.is_open())
	{
		using namespace boost;
		char_separator<char> sep(" ");
...
bool BeeGraph::load_map_from_jsonstr(std::string json_str, double left_w_weight, double right_w_weight)
{
    throw std::runtime_error("BeeGraph::load_map_from_jsonstr Not implemented");
}

void BeeGraph::reset_weights(bool consider_rotation, std::string log_dir, bool optimize_wait, std::vector<double>& weights)
{
    throw std::runtime_error("BeeGraph::reset_weights Not implemented");
}