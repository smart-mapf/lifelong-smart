...
Path SIPP::updatePath(const BasicGraph& G, SIPPNode* goal)
{
    ...
    while (true)
    {
        ...
        if (curr->parent == nullptr) // root node
        {
            ...
            if (curr->state.is_rotating)
            {
                ...
                while (t > 0)
                {
                    t--;
                    path[t] = State(
                        curr->state.location, t, curr->state.orientation, false, true);
                }
            }
        ...
        else
        {
            ...
            {
                path[t] = State(prev->state.location, t, curr->state.orientation, false, true);
                t++;
            ...
            {
                path[t] = State(prev->state.location, t, (prev->state.orientation + 1) % 4, false, true); // turn right
                t++;
                // Add rotational waiting states
                for (int i = 0; i < this->rotation_time - 1; i++)
                {
                    path[t] = State(prev->state.location, t, (prev->state.orientation + 1) % 4, false, true);
                    t++;
                }
                path[t] = State(prev->state.location, t, curr->state.orientation, false, true); // turn right
                t++;
                // Add rotational waiting states
                for (int i = 0; i < this->rotation_time - 1; i++)
                {
    ...
    while (!focal_list.empty())
    {
        ...
        if (!this->consider_task_wait)
        {
               ...
               curr->goal_id < (int)goal_location.size())
            {
                    ...
                    earliest_holding_time > curr->state.timestep)
                {
                    ...
                    bool can_wait = rt.canWaitUntil(
                        curr->state.location, wait_start_t, wait_until_t);
                    if (can_wait)
                    {
                        curr->state.timestep += curr_goal.task_wait_time;
        ...
        if (curr_goal.orientation != -1 &&
            ...
            this->rotation_time <=
                std::get<1>(curr->interval) - curr->state.timestep)
        {
            double h_val = compute_h_value(G, curr->state, curr->goal_id,
                                           goal_location);
            int degree = 1;
            int min_timestep =
                curr->state.timestep + degree * this->rotation_time;
            for (auto interval : rt.getSafeIntervals(curr->state.location,
                    min_timestep, std::get<1>(curr->interval) + 1))
            {
                generate_node(interval, curr, G, curr->state.location,
                    min_timestep, (curr->state.orientation + 1) % 4, h_val);
                generate_node(interval, curr, G, curr->state.location,
                    min_timestep, (curr->state.orientation + 3) % 4, h_val);
            }
        }
