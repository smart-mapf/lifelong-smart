...
bool SortingGrid::get_r_mode() const
{
	return this->r_mode;
}


bool SortingGrid::get_w_mode() const
{
	return this->w_mode;
}

int SortingGrid::get_n_valid_edges() const
{
    return this->n_valid_edges;
}


bool SortingGrid::load_map_from_jsonstr(
...
*/
void SortingGrid::infer_sim_mode_from_map(json G_json)
{
	this->r_mode = false;
	this->w_mode = false;
	std::string line;
...
void SortingGrid::parseMap(std::vector<std::vector<double>>& map_e, std::vector<std::vector<double>>& map_w){
    map_e.clear();
...
void SortingGrid::update_task_dist(std::mt19937& gen, std::string task_dist_type){
...
bool SortingGrid::load_unweighted_map_from_json(
    json G_json,
    double left_w_weight,
    double right_w_weight)
{
...
void SortingGrid::update_map_weights(bool optimize_wait,
...
bool SortingGrid::load_weighted_map_from_json(
    json G_json,
    double left_w_weight,
    double right_w_weight)
{
    ...
    vector<double> new_weights = G_json["weights"];
    this->update_map_weights(G_json["optimize_wait"], new_weights);
...
bool SortingGrid::load_weighted_map(std::string fname)
{
	std::string line;
...
// load map
bool SortingGrid::load_unweighted_map(
    std::string fname,
    double left_w_weight,
    double right_w_weight)
{
...
void SortingGrid::preprocessing(bool consider_rotation, std::string log_dir)
{
...
void SortingGrid::reset_weights(bool consider_rotation, std::string log_dir, bool optimize_wait, std::vector<double>& weights)
{
	// update weights
...
double SortingGrid::get_avg_task_len(
...
void SortingGrid::get_adj_endpoints()
{
    ...
    for (unsigned int i = 0; i < this->obstacles.size(); i++)
    {
        ...
        for (int d = 0; d < 4; d++)
        {
            int dir = dirs[d];
            int ep = obs + dir;
            if (0 <= ep && ep < this->cols * this->rows &&
                get_Manhattan_distance(obs, ep) <= 1 &&
                this->types[ep] == "Endpoint")
            {
