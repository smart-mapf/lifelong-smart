...
void BeeSystem::initialize_start_locations()
{
	for (int k = 0; k < (int)G.initial_locations.size(); k++)
	{
		...
		finished_tasks[k].emplace_back(Task(loc, -1, 0, 0, k, 0));
	}
...
void BeeSystem::update_goal_locations()
{
	for (int k = 0; k < num_of_drives; k++)
	{
		...
		int min_timesteps = max(G.get_Manhattan_distance(curr, goal.location), // cannot use h values, because graph edges may have weights
			goal.hold_time - timestep);
			...
			// The agent might finish its tasks during the next planning horizon
		{
            ...
            goal = Task(next.first, -1, 0, next.second);
			goal_locations[k].emplace_back(goal);
			min_timesteps += G.get_Manhattan_distance(
                next.first, goal.location);
			min_timesteps = max(min_timesteps, goal.hold_time - timestep);
			// goal = make_tuple(next.first, next.second, 0);
		}
		if (goal_locations[k].empty())
		{
			goal_locations[k].emplace_back(G.entrance, 0, 0);
		}
	}
}
...
void BeeSystem::simulate()
{
	...
	for (; timestep < simulation_time; timestep += simulation_window)
	{
		...
		for (auto task : new_finished_tasks)
		{
            ...
            loc = task.location;
            t = task.finish_t;
			if (loc != G.entrance || finished_tasks[id].back().location != G.entrance)
			{
				finished_tasks[id].emplace_back(task);
				if (loc != G.entrance)
					num_of_tasks++;
			}
		}
...
int BeeSystem::get_num_of_missed_tasks() const
{
	...
	for (auto finished_task : finished_tasks)
	{
		for (auto task : finished_task)
		{
			...
			{
				if (task.location == G.flowers[i] && task.finish_t > G.flower_time_windows[i].second)
				{
					missed_tasks++;
					break;
				}
...
int BeeSystem::get_num_of_remaining_tasks() const
{
	...
	for (const auto& tasks : finished_tasks)
	{
		...
		while (task != tasks.rend() && task->location != G.entrance) // robot fails to carry the items back to the entrance
		{
			remaining_tasks++;
			task++;
		}
...
list<int> BeeSystem::get_missed_flower_ids() const
{
	...
	for (auto finished_task : finished_tasks) // finished tasks
	{
		...
		while (task != finished_task.rend() && task->location != G.entrance) // robot fails to carry the items back to the entrance
		{
			for (int i = 0; i < (int)G.flowers.size(); i++)
			{
				if (task->location == G.flowers[i])
				{
					ids.push_back(i + 1);
					break;
				}
		...
		while (task != finished_task.rend()) // robot reaches the task after its deadline
		{
			for (int i = 0; i < (int)G.flowers.size(); i++)
			{
				if (task->location == G.flowers[i])
				{
					if (task->finish_t > G.flower_time_windows[i].second)
						ids.push_back(i + 1);
					break;
				}
			}
...
json BeeSystem::warmup(int warmup_time)
{
    throw std::runtime_error("BeeSystem::warmup Not implemented");
}

json BeeSystem::update_gg_and_step(int update_gg_interval)
{
    throw std::runtime_error("BeeSystem::update_gg_and_step Not implemented");
}

void BeeSystem::set_total_sim_time(int total_sim_time, int warmup_time)
{
    throw std::runtime_error("BeeSystem::set_total_sim_time Not implemented");
}