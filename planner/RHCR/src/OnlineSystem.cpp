...
void OnlineSystem::update_start_and_goal_locations(int num_of_new_agents)
{
	...
	while (count < num_of_new_agents && !available_entries.empty())
	{
		...
		goal_locations.back().emplace_back(G.exits[idx], 0, 0);
		count++;
	}
...
void OnlineSystem::move()
{
	...
	for (int t = start_timestep; t <= end_timestep; t++)
	{
		...
		while (path != paths.end())
		{
			...
			if ((int)path->size() == t + 1 &&
                path->back().location == goal->at(0).location)
			{
...
json OnlineSystem::warmup(int warmup_time)
{
    throw std::runtime_error("OnlineSystem::warmup Not implemented");
}

json OnlineSystem::update_gg_and_step(int update_gg_interval)
{
    throw std::runtime_error("OnlineSystem::update_gg_and_step Not implemented");
}

void OnlineSystem::set_total_sim_time(int total_sim_time, int warmup_time)
{
    throw std::runtime_error("OnlineSystem::set_total_sim_time Not implemented");
}