    ...
    const vector<int>& waited_time)
{
    ...
    path_planner.consider_task_wait = consider_task_wait;
    if (solution.size() == 0)
    ...
    std::vector<int> parking_agents;
    std::vector<int> non_parking_agents;
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
        if (std::find(parking_agents.begin(), parking_agents.end(), i) == parking_agents.end())
        {
            non_parking_agents.push_back(i);
        }
    }

    // if (screen > 1)
    // {
    //     // Print start and goals for each agent
    //     for (int i = 0; i < num_of_agents; i++)
    //     {
    //         std::cout << "Agent " << i << ": Start: " << starts[i].location
    //                   << ", Goals: ";
    //         for (const auto& goal : goal_locations[i])
    //         {
    ...
    // End the search if the time limit is reached or the number of iterations
    // exceeds the limit
    while (runtime < time_limit && num_restarts < n_iter_limit)
    {
        num_restarts++;
        // generate random priority order
        // Force non-parking agents to be first
        std::shuffle(non_parking_agents.begin(), non_parking_agents.end(),
                     this->gen);
        std::copy(non_parking_agents.begin(), non_parking_agents.end(),
                  priorities.begin());
        // Then shuffle parking agents
        std::shuffle(parking_agents.begin(), parking_agents.end(), this->gen);
        std::copy(parking_agents.begin(), parking_agents.end(),
                  priorities.begin() + non_parking_agents.size());
        // std::shuffle(priorities.begin(), priorities.end(), this->gen);
        ...
        for (int i : priorities)
        {
            ...
            // {
            //     // Print goals
            //     cout << "Planing agent " << i << " from " << starts[i].location
            //             << " to ";
            //     for (const auto& goal : goal_locations[i])
            //     {
            //         cout << goal.location << " ";
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
        if (solution[i].empty())
        {
            // If solution is empty, it will raise issue for the downstream
            // LRA*. Here we find a solution ignoring the constraints.
            solution[i] = path_planner.run(G, starts[i], goal_locations[i], rt);
        }
    }
...
void WHCAStar::print_results() const
{
    ...
    if(solution_cost >= 0) // solved
        std::cout << "Succeed,";
    else if (this->num_restarts >= this->n_iter_limit) // time_out or n-iter-out
        std::cout << "N_Iterations_Out,";
    else
        std::cout << "Timeout,";
