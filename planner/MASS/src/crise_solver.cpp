...
CRISE_Solver::CRISE_Solver(
    ...
    for (auto& sipp_interval : sipp_result_nodes) {
        ...
        if ((tmp_interval->t_max - tmp_interval->t_min) <
            (2 * curr_agent.length + CELL_DIS) / bot_motion->V_MAX) {
        ...
        if (DEBUG_CRISE)
            printf("Forward interval at: %d, from %f to %f\n",
                   tmp_interval->location, tmp_interval->t_min,
                   tmp_interval->t_max);
 ...
 * @return
 */
std::vector<std::pair<double, double>> CRISE_Solver::occupancyTimes(
    int length) {
...
bool CRISE_Solver::solve(shared_ptr<MotionNode>& solution) {
    ...
    for (int i = 0; i < length; i++) {
        double t_lower =
            std::max(0.0, result_nodes[i]->t_min - t_bound[i].first);
        double t_upper =
            std::min(INF, result_nodes[i]->t_max - t_bound[i].second);
        if (t_upper < t_lower) {
            return false;
        }
        if (wait_t < t_lower) {
    ...
    for (int i = 0; i < length; i++) {
        ...
        solution->local_path.emplace_back(result_nodes[i]->location, curr_o,
                                          t_lower, t_upper);
