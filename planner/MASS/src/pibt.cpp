...
bool PIBT::run(const vector<State> &starts,
    ...
    // Also remember the goal locations. Handy for converting the solution
    ...
    for (int i = 0; i < num_of_agents; i++) {
        ...
        // given timestep
            ...
            actual_start.orientation = orient::None;  // No orientation in PIBT
    ...
    for (int t = 1; t < this->simulation_window + 1; t++) {
        ...
        auto agents_cmp = [&](int a, int b) {
            ...
            if (this->goals_mem[a].empty() || this->goals_mem[b].empty())
                return false;
            double dist_a = this->G->getHeuristicOneGoalPebbleMotion(
                this->goals_mem[a][0].loc, solution[a][t - 1].location,
                solution[a][t - 1].orientation);
            double dist_b = this->G->getHeuristicOneGoalPebbleMotion(
                this->goals_mem[b][0].loc, solution[b][t - 1].location,
        ...
        if (screen > 1) {
            ...
            for (int i = 0; i < num_of_agents; i++) {
                ...
                for (const auto &state : solution[i]) {
                    cout << "(" << this->G->getRowCoordinate(state.location)
                         ...
                         << "," << state.timestep << "," << state.orientation
            ...
            for (int i = 0; i < num_of_agents; i++) {
                ...
                for (const auto &goal : this->goals_mem[i]) {
                    // cout << goal.loc << " ";
                    cout << "(" << this->G->getRowCoordinate(goal.loc) << ","
                         << this->G->getColCoordinate(goal.loc) << ") -> ";
                }
                cout << endl;
            }
...
bool PIBT::pibt_funct(int a_i, int a_j, State start_state, Task goal,
    ...
    //         double cost1 =
    //             this->G->getWeight(start_state.location, n1.location);
    //         double cost2 =
    //             this->G->getWeight(start_state.location, n2.location);
    ...
    //         this->G->getWeight(start_state.location, n2.location);
    //     double h1 =
    //         this->G->getHeuristicOneGoalPebbleMotion(goal.location,
    //         n1.location, n1.orientation);
    //     double h2 =
    //         this->G->getHeuristicOneGoalPebbleMotion(goal.location,
    //         n2.location, n2.orientation);
    ...
    if (this->is_waiting[a_i]) {
        // If the agent is waiting, it can only wait in place
        next_states.clear();
        next_states.push_back(start_state.wait());
    } else {
        list<int> next_locs = this->G->getNeighbors(start_state.location);
        next_locs.push_back(start_state.location);  // Include the wait action
        for (int loc : next_locs) {
            next_states.push_back(
                State(loc, start_state.timestep + 1, start_state.orientation));
        }
    }
    ...
    auto action_cmp = [&](const State &n1, const State &n2) {
        ...
        double h1 = this->G->getHeuristicOneGoalPebbleMotion(
            goal.loc, n1.location, n1.orientation);
        double h2 = this->G->getHeuristicOneGoalPebbleMotion(
            goal.loc, n2.location, n2.orientation);

        // Divide non-waiting heuristic by 2. This is because the heuristic of
        // MASS are all multiplied by 2 to get empirical scalability.
        if (n1.location != start_state.location)
            h1 /= 2;
        if (n2.location != start_state.location)
            h2 /= 2;
    ...
    // Print next states
    if (screen > 1) {
        cout << "Next states for agent " << a_i << " at timestep " << from_t
             << ": " << endl;
        for (const auto &next_state : next_states) {
            cout << next_state << " (cost: "
                 << this->G->getWeight(start_state.location,
                                       next_state.location)
                 << ", heuristic: "
                 << this->G->getHeuristicOneGoalPebbleMotion(
                        goal.loc, next_state.location, next_state.orientation)
                 << ")" << endl;
        }
        cout << endl;
    }
