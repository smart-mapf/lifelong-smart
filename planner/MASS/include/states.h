...
struct State {
    ...
    State wait() const {
        return State(location, timestep + 1, orientation);
    }
    ...
    State()
        : location(-1),
          ...
          is_tasking_wait(false),
          is_rotating(false) {
    }
    // State(int loc): loc(loc), timestep(0), orientation(0) {}
    // State(int loc, int timestep): loc(loc), timestep(timestep),
    // orientation(0) {} State(
    //     int location,
    //     int timestep = -1,
    ...
    State(int location, int timestep = -1, orient orientation = orient::None,
          bool is_tasking_wait = false, bool is_rotating = false)
        : location(location),
          timestep(timestep),
          orientation(orientation),
