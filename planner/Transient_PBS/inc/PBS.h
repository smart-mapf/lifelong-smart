...
public:
    ...
    // Returns each agent's path as a list of <x, y, time, task_id>
    std::vector<std::vector<std::tuple<int, int, double, int>>> getPaths();
...
private:
    ...
    // [i][j] = true indicates that i is lower than j
