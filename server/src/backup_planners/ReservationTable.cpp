    ...
    if (sit.find(location) == sit.end()) {
        ...
        if (it != ct.end()) {
            for (auto time_range : it->second)
                insertConstraint2SIT(location, time_range.first,
                                     time_range.second);
            ct.erase(it);
        }
...
void ReservationTable::insertSoftConstraint2SIT(int location, int t_min,
    ...
    for (auto it = sit[location].begin(); it != sit[location].end(); it++) {
        ...
        if (std::get<0>(*it) < t_min && std::get<1>(*it) <= t_max) {
            sit[location].insert(it,
                                 make_tuple(std::get<0>(*it), t_min, false));
...
void ReservationTable::build(
    ...
    for (auto con : hard_constraints) {
        // positive constraint
        if (std::get<0>(con) == current_agent && std::get<4>(con)) {
...
void ReservationTable::insertConstraints4starts(const vector<Path*>& paths,
    ...
    for (int i = 0; i < num_of_agents; i++) {
        if (paths[i] == nullptr)
            continue;
        // prohibit the agent from conflicting with other agents at their start
        // locations
            ...
            for (auto state : (*paths[i])) {
                ...
                {
                    // The agent waits at its start locations between
                    // [appear_time, state.timestep - 1] So other agents cannot
                    // use this start location between [appear_time - k_robust,
                    // state.timestep + k_robust - 1]
...
// Starting from the given safe interval, find all connected safe intervals,
// find the t-max of the last safe interval, check if t-max > desired t-max
bool ReservationTable::canWaitUntil(int location, int desired_t_min,
                                    int desired_t_max) {
    auto all_intervals = getSafeIntervals(location, 0, INTERVAL_MAX);

    // auto interval_it = std::find(all_intervals.begin(), all_intervals.end(),
    // curr_interval); Find the the first interval in all_intervals, where t_min
    // <= desired_t_min < t_max
    auto interval_it =
        std::find_if(all_intervals.begin(), all_intervals.end(),
                     [desired_t_min](const Interval& interval) {
                         return std::get<0>(interval) <= desired_t_min &&
                                desired_t_min < std::get<1>(interval);
                     });

    // Cannnot find the specified interval. Cannot wait
    if (interval_it == all_intervals.end()) {
        return false;
    }

    // Starting from interval_it, increment if the next safe interval is
    // connected with the current
    int curr_t_max = std::get<1>(*interval_it);
    ++interval_it;
    // Continue the while loop until the next safe interval is not connected
    while (std::get<0>(*interval_it) == curr_t_max) {
        curr_t_max = std::get<1>(*interval_it);
        ++interval_it;
    }

    // Check if the last safe interval's t_max is later than the desired
    return curr_t_max > desired_t_max;
}
...
bool ReservationTable::isConflicting(int curr_id, int next_id,
    ...
    // check vertex constraints (being in next_id at next_timestep is
    // disallowed)
    if (cat[next_timestep][next_id])
        return true;
    // check edge constraints (the move from curr_id to next_id at
    // next_timestep-1 is disallowed) which means that res_table is occupied
    // with another agent for [curr_id,next_timestep] and
    // [next_id,next_timestep-1] WRONG!
