...
bool LRAStar::run(const vector<State> &starts,
    ...
    if (this->num_of_agents != starts.size()) {
        spdlog::error(
            "The number of starts ({}) is not equal to the number of agents "
            "({})",
            starts.size(), this->num_of_agents);
        throw std::runtime_error("The number of starts is not correct");
    }

    num_expanded = 0;
    num_generated = 0;
    // Plan individual paths, if necessary
    vector<Path> shortest_paths = guide_paths;
    for (int i = 0; i < num_of_agents; i++) {
        // runtime = (std::clock() - start) * 1.0 / CLOCKS_PER_SEC;
        // if (runtime > time_limit)
        //     return false;
        if (shortest_paths.at(i).empty()) {
            spdlog::warn(
    ...
    if (screen > 0) {
        ...
        if (!this->validate_solution()) {
            spdlog::error("LRA* found an invalid solution.");
            exit(-1);
        }
    }
    runtime = (std::clock() - start) * 1.0 / CLOCKS_PER_SEC;
...
Path LRAStar::find_shortest_path(const State &start,
    ...
    double h_val = path_planner.compute_h_value(G, this->heuristic_table, start,
                                                0, goal_locations);
    ...
    while (!open_list.empty()) {
        ...
        if (curr->state.location == curr_goal.location &&
            ...
            if (curr->goal_id == (int)goal_locations.size()) {
                ...
                while (curr != nullptr) {
                    int t_min = curr->parent == nullptr
                                    ? 0
                                    : curr->parent->state.timestep + 1;
        ...
        for (const auto &next_state : G.get_neighbors(curr->state)) {
            ...
            double next_h_val = path_planner.compute_h_value(
                G, this->heuristic_table, next_state, curr->goal_id,
                goal_locations);
            // This vertex cannot reach the goal vertex
            if (next_h_val >= WEIGHT_MAX)
                continue;
...
void LRAStar::resolve_conflicts(const vector<Path> &input_paths) {
    ...
    for (int t = 1; t <= simulation_window; t++) {
        ...
        for (auto agent : agents_list) {
            ...
            int orientation =
                input_paths[agent][path_pointers[agent]].orientation;
            auto is_tasking_wait =
                input_paths[agent][path_pointers[agent]].is_tasking_wait;
            ...
            if (loc == prev_loc) {
                ...
                path_pointers[agent]++;
                auto other =
                    next_locations.find(loc);  // conflict with other agent
                if (other != next_locations.end()) {
                    // Other agent has to wait
                    wait_command(other->second, t, path_pointers);
                    path_pointers[other->second]--;
                }
            }
            // Resolve following conflict
            // The agent cannot move because its next location is occupied
            // currently
            else if (k_robust == 1 &&
                     curr_locations.find(loc) != curr_locations.end()) {
                wait_command(agent, t, path_pointers);
            }
            // The agent wants to move from prev_loc to loc
            else {
...
//             int loc = input_paths[agent][path_pointers[agent]].location;
//             int orientation =
//             input_paths[agent][path_pointers[agent]].orientation; auto
//             is_tasking_wait =
//             input_paths[agent][path_pointers[agent]].is_tasking_wait; if (loc
//             == solution[agent][t - 1].location) { // The agent wait or
//             rotates at its current location
//                 solution[agent].emplace_back(loc, t, orientation,
//                 is_tasking_wait); path_pointers[agent]++; auto other =
//                 next_locations.find(loc); // conflict with other agent if
//                 (other != next_locations.end())
//                 {
//                     wait_command(other->second, t, path_pointers); // Other
//                     agent has to wait path_pointers[other->second]--;
//                 }
//             }
...
//                 wait_command(agent, t, path_pointers);
//             }
//             else // The agent wants to move to a location that is currently
//             empty
//             {
...
//                     solution[agent].emplace_back(loc, t, orientation,
//                     is_tasking_wait); path_pointers[agent]++;
//                 }
//                 else
//                 { // Another agent also wants to go to this location, for now
//                 we just force this agent to wait
//                     wait_command(agent, t, path_pointers);
//                 }
//             }
//             next_locations[solution[agent][t].location] = agent;
...
//                            vector<list<pair<int, int>>::const_iterator>
//                            &traj_pointers)
// {
...
//     auto other = next_locations.find(location); // whether conflict with
//     other agent if (other != next_locations.end())
//     {
//         wait_command(other->second, timestep, traj_pointers); // Other agent
//         has to wait
...
void LRAStar::print_results() const {
    spdlog::info("LRA*:Succeed,Runtime={},# wait commands={}", runtime,
                 num_wait_commands);
}
