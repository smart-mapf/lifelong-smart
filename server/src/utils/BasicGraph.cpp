...
//         // To preserve order, create a vector, store the heuristics there,
//         and
...
//     // std::cout << "start computing h for loc = "<< root_location
//     <<std::endl; std::vector<double> res; if (consider_rotation) {
//         // If considering rotation, we need to have 4 times the size of the
//         grid res.resize(this->size() * 4, WEIGHT_MAX);
//     } else {
//         // If not considering rotation, we only need the size of the grid
//         res.resize(this->size(), WEIGHT_MAX);
//     }
...
//         root->open_handle = heap.push(root);  // add root to heap
//         nodes.insert(root);                   // add root to hash_table
//         (nodes)
//     }
...
//                 new StateTimeAStarNode(next_state, next_g_val, 0, nullptr,
//                 0);
...
// vector<double> BasicGraph::compute_pebble_motion_heuristics(int
// root_location) {
//     // While not considering rotation, the pebble motion heuristic is the
//     same
...
//     // std::cout << "start computing h for loc = "<< root_location
//     <<std::endl; std::vector<double> res(this->size(), WEIGHT_MAX);
...
//             {}", start_loc,
//             this->pebble_motion_heuristics.at(goal_loc).size());
//         exit(1);
//     }
...
//         if (consider_rotation) {
//             double min_heuristic = WEIGHT_MAX;
