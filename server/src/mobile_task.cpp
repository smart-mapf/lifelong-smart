...
MobileTaskManager::MobileTaskManager(int num_agents, int num_picker,
                                     std::string& map_fname)
    : TaskManager(num_agents, map_fname), num_picker_(num_picker) {
...
void MobileTaskManager::cleanTasks() {
    for (int i = 0; i < num_picker_; ++i) {
        ...
        while (not all_picker_tasks[i].empty() and
               (all_picker_tasks[i].front()->status == DELIVER or
                all_picker_tasks[i].front()->status == DONE)) {
            all_picker_tasks[i].pop_front();
        }
    }
}

void MobileTaskManager::getTask(
    const std::vector<std::pair<double, double>>& robots_location,
    ...
    if (not free_agents.empty() and not front_tasks.empty()) {
        ...
        for (int robot_id : free_agents) {
            ...
            for (int j = 0; j < front_tasks.size(); ++j) {
                double tmp_dis = computeDistance(robots_location[robot_id],
                                                 front_tasks[j]->goal_position);
        ...
        std::cout << "Finish solve hungarian, the cost is: " << cost
                  << std::endl;
        for (int i = 0; i < assignment.size(); ++i) {
            if (assignment[i] >= 0) {
                cout << "Agent " << free_agents[i] << " assigned to Task "
                     << assignment[i] << endl;
...
void MobileTaskManager::finishTask(int agent_id,
                                   std::shared_ptr<MobileRobotTask>& task) {
    if (task->status == PICK) {
        setTask(agent_id, task, DELIVER);
    } else if (task->status == DELIVER) {
        setTask(agent_id, task, DONE);
    } else {
        std::cerr << "MobileTaskManager::finishTask: unknown action"
                  << std::endl;
        exit(-1);
    }
}

void MobileTaskManager::setTask(int agent_id,
                                std::shared_ptr<MobileRobotTask>& task,
                                MobileAction status) {
    if (task->status == NONE) {
        if (status == PICK) {
        ...
        if (status == DONE) {
            if (not agent_task_status[agent_id].assigned_tasks.empty()) {
                if (agent_task_status[agent_id].assigned_tasks.front()->id ==
                    task->id) {
                    agent_task_status[agent_id].assigned_tasks.pop_front();
                    total_finished_tasks_++;
                    std::cout << "Total number of tasks finished: "
                              << total_finished_tasks_ << " finished"
                              << std::endl;
                }
            }
        } else {
            // std::cerr << "Its a DELIVER task, but got " << status <<
            // std::endl; exit(-1);
            return;
        }
...
std::pair<int, int> MobileTaskManager::findPalletizer(int picker_id) {
    ...
    // std::srand(std::time(0)); // Seed the random number generator with
    // current time station_idx = std::rand() % active_stations.size();
    std::cout << "active stations size: " << active_stations.size()
              << ", picked station idx: " << station_idx << std::endl;
...
int MobileTaskManager::insertPickerTask(int picker_id, int goal_x, int goal_y) {
    ...
    std::shared_ptr<MobileRobotTask> new_pickup_task =
        std::make_shared<MobileRobotTask>(curr_task_idx++, -1, free_loc, -1,
                                          MobileAction::NONE, picker_id,
                                          picker_loc);
