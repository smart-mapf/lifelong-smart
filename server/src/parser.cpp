...
// Processes the actions of an agent, calculating necessary steps, including orientation changes.
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        if (i == 0) {
            ...
            if (neededOrientation != currentOrientation) {
                // Change orientation without advancing time.
...
// Processes the actions of an agent for the continuous case, using provided timestamps.
void processAgentActionsContinuous(const vector<Point>& points, vector<Step>& steps, int agentId) {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        if (i == 0) {
            ...
            // Move to the next position, use the provided timestamp.
            currentTime = points[i].time;
            steps.push_back({points[i].x, points[i].y, currentOrientation, currentTime});
        }
    }
...
// Parses a single line of input to extract x, y coordinates and assigns sequential times starting at 1.0.
vector<Point> parseLine(const string& line) {
    ...
    ss.ignore(100, ':'); // Ignore characters until ':' which precedes the data.

    while (ss >> ignore >> x >> ignore >> y >> ignore) {
        points.push_back({x, y, time++});
        ss >> ignore >> ignore; // Skip "->" characters.
    }

    return points;
}

// Parses a single line of input to extract x, y, t coordinates for the continuous case.
    ...
    if (file_type == DISCRETE) {
        ...
        while (getline(inFile, line)) {
            ...
            if (!line.empty()) {
                ...
                processAgentActions(points, tmp_plan, 0, agentId++);
            }
